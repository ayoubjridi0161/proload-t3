...
export default async function page({}: Props) {
    ...
    const parsedPosts = posts.map((post)=> ({id:post.id ,likesCount:post.likes, title:post.title, content:post.content , user:post.users.name , userImage:post.users.image , userId:post.userId , media : post.resources}))
    ...
    <main className='flex gap-2 justify-around max-h-full'>
      ...
      <section className='w-2/3 p-5 overflow-y-scroll'>
        ...
        {parsedPosts?.map((post)=> <Post media = {post.media} likes={post.likesCount} key={post.id} id={post.id} userImage={post.userImage ?? ""} liked={likes?.includes(post.id)} userName={post.user ?? ""} userId={post.userId ?? -1} title={post.title} postContent={post.content} />)}
      </section>
